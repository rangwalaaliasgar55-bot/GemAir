...
async function findPython(options = {}) {
  ...
  for (const [command, prefix] of candidates) {
    try {
      ...
      if (match && Number(match[1]) === 3 && Number(match[2]) >= 10 && Number(match[2]) < 14) return { command, prefix };
    } catch {}
  }
...
async function start(options = {}) {
  ...
  startPromise = (async () => {
    ...
    child.once('exit', (code, signal) => {
      const error = new Error(`OPENJARVIS_EXIT_${code == null ? signal : code}${stderr ? ': ' + stderr.slice(-1000) : ''}`);
      lastError = error.message;
      if (processHandle === child) processHandle = null;
      rejectPending(error);
    });

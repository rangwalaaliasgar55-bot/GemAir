...
async function loginChatGPTViaPkce(openBrowser) {
  ...
  try { tokens = await loginChatGPT({ openBrowser }); }
    ...
    if (/invalid_authorize_request|invalid.*client|unauthorized_client|missing_required_parameter/i.test(error.message || '')) return { error: 'CHATGPT_OAUTH_CLIENT_REJECTED', message: 'OpenAI rejected this OAuth request. Retry — if it persists, use Import Codex login below instead.' };

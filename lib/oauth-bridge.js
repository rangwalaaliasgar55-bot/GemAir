...
async function loginChatGPTViaPkce(openBrowser) {
  ...
  try { tokens = await loginChatGPT({ openBrowser }); }
    ...
    if (/invalid_authorize_request|invalid.*client|unauthorized_client|missing_required_parameter/i.test(error.message || '')) return { error: 'CHATGPT_OAUTH_CLIENT_REJECTED', message: 'OpenAI rejected this sign-in request. Use Paste session JSON in Settings instead — it only needs your ChatGPT login.' };
...
// Refresh window: start refreshing this far before access-token expiry.
const REFRESH_SKEW_MS = 5 * 60 * 1000;

/**
 * Proactively refresh the stored ChatGPT session when it is missing an
 * expiry or expires within the refresh window. store/fetchFn/nowMs default
 * to the real encrypted store, network fetch, and clock, but are injectable
 * so tests run without Electron or network.
 */
async function checkAndRefreshChatGPT(opts = {}) {
  ...
  if (tokens.expiresAt && tokens.expiresAt - nowMs > REFRESH_SKEW_MS) {
    return { refreshed: false, reason: 'NOT_DUE' };
  }
  const { refreshChatGPTAccessToken } = require('./oauth-chatgpt-pkce');
  try {
    const data = await refreshChatGPTAccessToken(tokens.refreshToken, opts.fetchFn);
    ...
    return { refreshed: false, code: error.code || 'REFRESH_FAILED', message: error.message };
  }
}

module.exports = { loginChatGPTViaPkce, loginGeminiViaPkce, importChatGPTFromCodex, checkAndRefreshChatGPT, REFRESH_SKEW_MS };

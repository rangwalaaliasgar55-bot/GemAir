...
const EXCALIDRAW = {
    ...
    steps: [
        ...
        { id: "check-node", title: "Check Node", kind: "command", command: "node --version", check: { type: "tool_version", tool: "node" } },
        {
            ...
            kind: "command",
            command: "cd ~; git clone https://github.com/excalidraw/excalidraw.git",
            // Idempotent on macOS/Linux so a re-run does not die on an existing clone.
            posixCommand: "mkdir -p ~/gemair-apps; cd ~/gemair-apps; [ -d excalidraw ] || git clone https://github.com/excalidraw/excalidraw.git",
            workingDirectory: "~",
        },
        {
            id: "dependencies",

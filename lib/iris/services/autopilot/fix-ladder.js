...
//
//   - The transport is `MaintainModelProviding.respond()` (the same free
//     OpenCode seam maintain mode already uses — never a funded proxy, never a
//     paid model), driven with the *Codex* plain-text contract: the
...
class FixLadder {
    ...
    /// Who pays for the model calls. In GemAir this is ALWAYS the default —
    /// every route is a free OpenCode model on the reader's own account, so
    /// nobody is metering spend and the spend caps do not apply:
    /// `mayAskTheModelAgain` never surfaces "used up what I can spend" for money
    /// nobody was spending. The progress guard still bounds a runaway. The
    /// `metered_tier` branch is upstream's funded-proxy behaviour, kept only so
    /// a reader who wires in a paid provider of their own still gets a ceiling.
    ...
    ///
    /// The caps bound SPEND, so they only apply when somebody is metering it.
    /// GemAir's routes are free, so there is nothing to protect and this returns
    /// true unconditionally — the same answer macOS's
    /// `theLadderMayAskTheModelAgain` gives when nobody else is paying for the
    /// call about to be made. The per-step rung cap and the going-in-circles
    /// progress guard still bound a runaway; what is removed is only the "used
    /// up what I can spend" ceiling on money nobody is spending. Under a metered
    /// tier both counters increment together, so the fix cap (the lower) binds
    /// first.
    mayAskTheModelAgain() {

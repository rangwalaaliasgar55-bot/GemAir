...
class MaintainTierCFixer {
    ...
    async attemptFix(options) {
        ...
        const gitBackupPath = path.join(this.tempDirectoryPath, `gemair-git-backup-${signatureId.slice(0, 8)}`);
        ...
        const branchName = `gemair/fix-${signatureId.slice(0, 12)}-${dateStamp}`;
        const commitMessage = `Novel fix for ${appSlug}\n\n` +
            ...
            `Assisted-by: gemair-maintain-mode/1 (tier-c, ${this.provider.displayName})\n` +

...
class MaintainPoolClient {
    ...
    fetchImplementation;
    constructor(options = {}) {
        this.publikBaseUrl = (options.poolBaseUrl ?? options.publikBaseUrl ?? exports.DEFAULT_MAINTAIN_POOL_BASE_URL).replace(/\/+$/, "");
        ...
        this.fetchImplementation = this.publikBaseUrl
            ? injectedFetch
            : async () => {
    ...
    async topFeatureRequests(appSlug) {
        try {
            const response = await this.fetchImplementation(`${this.publikBaseUrl}/api/iris/feature-requests?${this.searchParams({ app: appSlug })}`, { method: "GET" });
            if (!response.ok) {

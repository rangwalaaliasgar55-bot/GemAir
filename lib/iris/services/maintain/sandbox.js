...
function buildSandboxIdentifiers(jailId, tempDirectoryPath) {
    const baseName = `gemair-jail-${jailId}`;
...
function buildJailedPowerShellScript(options) {
    ...
    return [
        ...
        "$job = [GemAirJail.Native]::CreateJobObject([IntPtr]::Zero, $jobObjectName)",
        "$limitInfo = New-Object GemAirJail.JOBOBJECT_EXTENDED_LIMIT_INFORMATION",
        "$limitInfo.BasicLimitInformation.LimitFlags = [GemAirJail.Native]::JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE -bor [GemAirJail.Native]::JOB_OBJECT_LIMIT_ACTIVE_PROCESS -bor [GemAirJail.Native]::JOB_OBJECT_LIMIT_JOB_MEMORY",
        ...
        "[GemAirJail.Native]::SetInformationJobObject($job, [GemAirJail.Native]::JobObjectExtendedLimitInformation, [ref]$limitInfo, $limitInfoSize) | Out-Null",
        "",
        ...
        "try {",
        "  [GemAirJail.Native]::OpenProcessToken([System.Diagnostics.Process]::GetCurrentProcess().Handle, [GemAirJail.Native]::TOKEN_ALL_ACCESS, [ref]$currentProcessToken) | Out-Null",
        "  $newRestrictedToken = [IntPtr]::Zero",
        "  $restrictedTokenCreated = [GemAirJail.Native]::CreateRestrictedToken($currentProcessToken, [GemAirJail.Native]::DISABLE_MAX_PRIVILEGE, 0, [IntPtr]::Zero, 0, [IntPtr]::Zero, 0, [IntPtr]::Zero, [ref]$newRestrictedToken)",
        ...
        "$sa = New-Object GemAirJail.SECURITY_ATTRIBUTES",
        "$sa.nLength = [System.Runtime.InteropServices.Marshal]::SizeOf($sa)",
        "$sa.bInheritHandle = $true",
        "$sa.lpSecurityDescriptor = [IntPtr]::Zero",
        "$shareMode = [GemAirJail.Native]::FILE_SHARE_READ -bor [GemAirJail.Native]::FILE_SHARE_WRITE",
        "$stdoutHandle = [GemAirJail.Native]::CreateFileW($stdoutFilePath, [GemAirJail.Native]::GENERIC_WRITE, $shareMode, [ref]$sa, [GemAirJail.Native]::CREATE_ALWAYS, [GemAirJail.Native]::FILE_ATTRIBUTE_NORMAL, [IntPtr]::Zero)",
        "$stderrHandle = [GemAirJail.Native]::CreateFileW($stderrFilePath, [GemAirJail.Native]::GENERIC_WRITE, $shareMode, [ref]$sa, [GemAirJail.Native]::CREATE_ALWAYS, [GemAirJail.Native]::FILE_ATTRIBUTE_NORMAL, [IntPtr]::Zero)",
        "",
        ...
        "",
        "  $startupInfo = New-Object GemAirJail.STARTUPINFO",
        "  $startupInfo.cb = [System.Runtime.InteropServices.Marshal]::SizeOf($startupInfo)",
        "  $startupInfo.dwFlags = [GemAirJail.Native]::STARTF_USESTDHANDLES",
        "  $startupInfo.hStdOutput = $stdoutHandle",
        "  $startupInfo.hStdError = $stderrHandle",
        "  $startupInfo.hStdInput = [IntPtr]::Zero",
        "  $processInfo = New-Object GemAirJail.PROCESS_INFORMATION",
        ...
        "  $creationFlags = [GemAirJail.Native]::CREATE_NO_WINDOW -bor [GemAirJail.Native]::CREATE_SUSPENDED",
        "  $commandLine = '\"' + $shellCopyPath + '\" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -EncodedCommand ' + $encodedInnerCommand",
        "  $created = [GemAirJail.Native]::CreateProcessAsUser($tokenForChild, $null, $commandLine, [ref]$sa, [ref]$sa, $true, $creationFlags, [IntPtr]::Zero, $repoRootPath, [ref]$startupInfo, [ref]$processInfo)",
        "  if (-not $created) {",
        "    $lastError = [System.Runtime.InteropServices.Marshal]::GetLastWin32Error()",
        '    throw "CreateProcessAsUser failed with Win32 error $lastError"',
        "  }",
        "  [GemAirJail.Native]::AssignProcessToJobObject($job, $processInfo.hProcess) | Out-Null",
        "  [GemAirJail.Native]::ResumeThread($processInfo.hThread) | Out-Null",
        "  [GemAirJail.Native]::WaitForSingleObject($processInfo.hProcess, [GemAirJail.Native]::INFINITE) | Out-Null",
        "  $exitCodeOut = 0",
        "  [GemAirJail.Native]::GetExitCodeProcess($processInfo.hProcess, [ref]$exitCodeOut) | Out-Null",
        "  $childExitCode = $exitCodeOut",
        "  [GemAirJail.Native]::CloseHandle($processInfo.hProcess) | Out-Null",
        "  [GemAirJail.Native]::CloseHandle($processInfo.hThread) | Out-Null",
        "} finally {",
        ...
        "  [GemAirJail.Native]::CloseHandle($stdoutHandle) | Out-Null",
        "  [GemAirJail.Native]::CloseHandle($stderrHandle) | Out-Null",
        "}",
        "",
        ...
        "if ($haveRestrictedToken) { [GemAirJail.Native]::CloseHandle($restrictedToken) | Out-Null }",
        ...
        "[GemAirJail.Native]::CloseHandle($job) | Out-Null",
        "",
        "exit $childExitCode",
        "",
    ].join("\n");

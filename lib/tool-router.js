...
const INTENT_HINTS = [
  ...
  { re: /\b(click|mouse|screen|screenshot|desktop|window|scroll|type|keyboard|press key)\b/i, tools: ['run_desktop_task', 'capture_agent_screen', 'describe_screen', 'move_mouse', 'mouse_click', 'type_text', 'press_key', 'scroll_mouse', 'take_screenshot', 'list_windows'] },
  // An outcome that needs a sequence of physical actions is exactly what the
  // autonomous agent is for; without this hint the router only ever offers the
  // single-action tools, so the model clicks once per turn and looks helpless.
  { re: /\b(fill|complete|set up|setup|change the|turn on|turn off|enable|disable|toggle|clean up|tidy|install|update the|open .* and |do this for me|handle it)\b/i, tools: ['run_desktop_task', 'apply_mode', 'control_system', 'focus_app', 'launch_app', 'open_site'] },

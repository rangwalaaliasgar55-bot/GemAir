...
function register(ipcMain, service, { broadcast, openIsland, setIslandVisible, openExternal, focusSubject } = {}) {
  ...
  // ---- tab awareness (the island's "what are you on?" line) ----
  // The main window reports its own active view; the island shows it next to the
  // OS focus so a glance answers both "where am I in GemAir" and "what tab is
  // actually in front of me on the machine".
  ipcMain.handle('air:setAppView', (_e, view) => service.setAppView(str(view, 40)));
  ipcMain.handle('air:tabs', (_e, limit) => ({ ok: true, tabs: service.tabs(Number(limit) || 6) }));
  ipcMain.handle('air:focusTab', async (_e, payload) => {
    const app = str(payload && payload.app, 80);
    if (!app) return { ok: false, error: 'app required' };
    if (typeof focusSubject !== 'function') return { ok: false, error: 'focus unavailable' };
    try {
      const result = await focusSubject({ app, title: str(payload && payload.title, 200), kind: str(payload && payload.kind, 20) });
      return result && result.error ? { ok: false, error: result.error } : { ok: true, result };
  ...
  ipcMain.handle('air:savePlan', (_e, plan) => {
    const p = {
      ...
      expiresAt: Number.isFinite(Number(plan && plan.expiresAt)) ? Number(plan.expiresAt) : null,
  ...
  ipcMain.handle('air:startFocus', (_e, minutes) => {
    ...
    const plan = {
      id: 'gem-air-quick-focus', name: 'Quick Focus', enabled: true, expiresAt: end.getTime(),
      days: Array.from(new Set([start.getDay(), end.getDay()])),
      blocks: [{ start: hhmm(start), end: hhmm(end), kind: 'focus', label: `Quick focus · ${duration} min` }],
      rules: { strict: false, protected: false, allowedApps: [], blockedApps: [], allowedSites: [], blockedSites: [], allowedCategories: [], blockedCategories: [] }
    };
    save((s) => { s.plans = s.plans.filter((item) => item.id !== plan.id); s.plans.push(plan); return s; });
    service.emitState();
    return { ok: true, plan, endsAt: end.toISOString() };
  });
  ...
  ipcMain.handle('air:openFocusx', (_e, path) => {
    const base = state().settings.focusxSite || 'https://focusarx.site';

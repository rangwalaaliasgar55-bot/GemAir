...
const fs = require('fs');
const { resolveSitePreset } = require('./site-presets');
...
async function snapWindow(direction) {
  ...
  const result = await execOut(ps);
  if (result.err) return { error: `Could not snap the active window: ${result.errOut || result.err.message}` };
...
async function minimizeAll() {
  ...
  let result;
  if (platform === 'win32') {
    result = await execOut(`powershell -NoProfile -Command "$shell = New-Object -ComObject Shell.Application; $shell.MinimizeAll()"`);
    return result.err ? { error: 'Could not minimize windows.' } : { ok: true, note: 'Minimized all windows (Windows)' };
  }
  if (platform === 'darwin') {
    result = await execFileOut('osascript', ['-e', 'tell application "System Events" to keystroke "m" using {command down, option down}']);
    return result.err ? { error: 'Could not minimize windows.' } : { ok: true, note: 'Minimized all (macOS)' };
  }
  result = await execOut(`xdotool key super+d 2>/dev/null || wmctrl -k on 2>/dev/null || echo no-tool`);
  if (result.err || result.out.includes('no-tool')) return { error: 'Minimize all requires xdotool or wmctrl on Linux.' };
  return { ok: true, note: 'Minimized all (Linux)' };
}
...
async function nextVirtualDesktop() {
  ...
  const result = await execOut(`powershell -NoProfile -Command "$wshell = New-Object -ComObject wscript.shell; $wshell.SendKeys('^#{RIGHT}')"`);
  return result.err ? { error: 'Could not switch virtual desktops.' } : { ok: true, note: 'Switched to next virtual desktop (Windows)' };
}

function normalizeWebUrl(value) {
  let text = String(value || '').trim();
  if (!text) return null;
  text = resolveSitePreset(text) || text;
...
async function openSite(url, browser) {
  ...
  try {
    if (!process.versions.electron) throw new Error('Electron shell unavailable');

...
function describeCodexError(error) {
  ...
  if (/CODEX_UNAUTHORIZED/.test(text)) return 'OpenAI rejected this session. Reconnect ChatGPT in Settings → AI & Connections.';
  if (/CODEX_MODEL_NOT_AVAILABLE/.test(text)) return 'OpenAI will not serve that model on this account (plans differ, and model names move). Pick another model in Settings → AI & Connections → ChatGPT model, or press Refresh models.';
  return '';
}
let sdkPromise = null;
...
async function callCodexResponses(options) {
  ...
  const send = async (payload) => {
    ...
    if (!response.ok) {
      ...
      err.detail = detail;
      // A plan/model rejection is a CONFIGURATION answer, not a dead session.
      // Distinguishing the two matters: the first means "pick another model",
      // the second means "sign in again" — and treating it as expiry used to
      // delete a healthy ChatGPT login over one unsupported model name.
      if (/model|not support|unsupported|no longer|does not exist|deprecat|invalid/i.test(detail) && [400, 404, 422].includes(response.status)) {
        err.code = 'CODEX_MODEL_NOT_AVAILABLE';
        err.retryableWithAnotherModel = true;
      }
      throw err;
    }
  ...
  try {
    ...
    if (error && error.retryableWithAnotherModel) {
      // Which models an account may use is plan-dependent and moves over time.
      // Try the rest of the account's discovered list instead of failing the
      // turn — the selected id may be one this plan stopped carrying.
      const candidates = [...new Set([
        ...(Array.isArray(options.availableModels) ? options.availableModels : []),
        ...(Array.isArray(options.modelCandidates) ? options.modelCandidates : [])
      ])].filter((m) => m && m !== body.model).slice(0, 3);
      for (const model of candidates) {
        try {
          ...
          // Auth or transport trouble on a fallback is the real signal — stop.
          if (!next || !next.retryableWithAnotherModel) throw next;
          error.detail = `${error.detail || ''} | also rejected for ${model}`.slice(0, MAX_ERROR_BODY);
        }
      }
      error.message = 'CODEX_MODEL_NOT_AVAILABLE';
      error.detail = ((error.detail || '') + '. ' + describeCodexError({ code: 'CODEX_MODEL_NOT_AVAILABLE' })).slice(0, MAX_ERROR_BODY);
    }
    throw error;
  }
}

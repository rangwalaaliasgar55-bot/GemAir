...
function collectEvent(state, event, onDelta) {
  ...
  if ((event.type === 'response.output_text.done' || event.type === 'response.content_part.done') && !state.text) {
    const text = typeof event.text === 'string' ? event.text
      : event.part && typeof event.part.text === 'string' ? event.part.text : '';
    if (text) { state.text = text; state.streamed = true; if (onDelta) onDelta(text); }
  }

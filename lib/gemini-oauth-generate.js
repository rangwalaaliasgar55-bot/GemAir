...
async function generateWithStoredGemini(prompt, model) {
  ...
  if (auth.mode === 'web-session') throw new Error('GEMINI_WEB_SESSION_ONLY: the stored value is a Google browser session, not an API credential. Save an AI Studio key instead.');
  if (auth.mode === 'key') {
    return connections.callGeminiWeb({ apiKey: auth.apiKey, messages: [{ role: 'user', content: prompt }] });
  }
  return generateGemini(tok.psid, prompt, model || 'gemini-2.5-flash');
}

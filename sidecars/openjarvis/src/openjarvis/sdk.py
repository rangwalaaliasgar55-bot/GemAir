...
class Jarvis:
    ...
    def _run_agent(
        ...
        *,
        ...
        # Build tools. GemAir's sidecar may include the private ``__mcp__``
        # sentinel to opt into the MCP servers that its host configuration has
        # explicitly allowlisted. The sentinel itself is never a tool.
        tool_objects: List[Any] = []
        mcp_clients: List[Any] = []
        use_mcp = "__mcp__" in tools
        local_tool_names = [name for name in tools if name != "__mcp__"]
        if local_tool_names:
        ...
        if use_mcp:
            ...
            mcp_tools, mcp_clients = load_mcp_tools_from_config(
                self._config.tools.mcp,
                allowed_names=None,
            )
            existing = {tool.spec.name for tool in tool_objects}
            for tool in mcp_tools:
                annotations = tool.spec.metadata.get("mcp_annotations", {})
                # GemAir's non-interactive reasoning bridge accepts only tools
                # that the MCP server declares read-only and non-destructive.
                # Mutating tools stay discoverable in Settings but are never
                # handed to an agent; host-side JS permission gates remain the
                # only route for concrete desktop changes.
                read_only = annotations.get("readOnlyHint") is True
                destructive = annotations.get("destructiveHint") is not False
                if read_only and not destructive and tool.spec.name not in existing:
        ...
        try:
            # The local mcp_clients list keeps transports strongly referenced
            # for the complete run; closing them here prevents session leaks.

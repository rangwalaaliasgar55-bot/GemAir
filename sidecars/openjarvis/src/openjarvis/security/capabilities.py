...
class CapabilityPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        # GemAir keeps the Rust extension optional: policy enforcement must
        # still fail closed on machines without a Rust toolchain. OpenJarvis's
        # Python implementation below mirrors the native checks and is used
        # until the optional extension is available.
        try:
    ...
    def check(self, agent_id: str, capability: str, resource: str = "") -> bool:
        ...
        *agent_id* has no explicit policy of its own. This lets a baseline be
        ...
        """
        check_impl = (
            self._rust_impl.check
            if self._rust_impl is not None
            else self._check_python
        )
        if (
            not agent_id
            ...
            return check_impl(agent_id, capability, resource)
        if self._DEFAULT_AGENT in self._policies:
            return check_impl(self._DEFAULT_AGENT, capability, resource)
        return check_impl(agent_id, capability, resource)

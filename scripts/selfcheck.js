...
try {
  ...
  if (pkg.version !== '2.12.0') fail(`package version must be 2.12.0 (found ${pkg.version})`);
  ...
  else ok('2.12.0 release metadata and platform icons present');
...
function printManualMatrix() {
  const rows = [
    ['1', 'Boot', 'Launch GemAir. Boot sequence completes; SYS chip reads SYSTEMS NOMINAL (or names degraded). Version tag v2.12.0'],

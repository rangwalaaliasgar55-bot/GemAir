...
try {
  ...
  if (pkg.version !== '2.5.3') fail(`package version must be 2.5.3 (found ${pkg.version})`);
  ...
  if (!fs.existsSync(path.join(ROOT, 'CHANGELOG.md')) || !read('CHANGELOG.md').includes('## [2.5.3]')) fail('CHANGELOG.md must document 2.5.3');
  else ok('2.5.3 release metadata and platform icons present');
...
function printManualMatrix() {
  const rows = [
    ['1', 'Boot', 'Launch GemAir. Boot sequence completes; SYS chip reads SYSTEMS NOMINAL (or names degraded). Version tag v2.5.3'],

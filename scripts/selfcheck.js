...
try {
  ...
  if (pkg.version !== '2.8.0') fail(`package version must be 2.8.0 (found ${pkg.version})`);
  ...
  if (!fs.existsSync(path.join(ROOT, 'CHANGELOG.md')) || !read('CHANGELOG.md').includes('## [2.8.0]')) fail('CHANGELOG.md must document 2.8.0');
  else ok('2.8.0 release metadata and platform icons present');
...
function printManualMatrix() {
  const rows = [
    ['1', 'Boot', 'Launch GemAir. Boot sequence completes; SYS chip reads SYSTEMS NOMINAL (or names degraded). Version tag v2.8.0'],
